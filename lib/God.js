...
God.handleExit = function handleExit(clu, exit_code, kill_signal) {
  ...
  var stopExitCodes = [];
  var stopExitCodesEnv = proc.pm2_env.stop_exit_codes;
  if (typeof stopExitCodesEnv === 'string') {
    stopExitCodes = stopExitCodesEnv
      .split(',')
      .map(code => parseInt(code, 10))
      .filter(code => !isNaN(code));
  } else if (stopExitCodesEnv instanceof Array) {
    stopExitCodes = stopExitCodesEnv
      .map(code => typeof code === 'string' ? parseInt(code, 10) : code)
      .filter(code => !isNaN(code));
  }

  var isStopExitCode = stopExitCodes.includes(exit_code);
  if (isStopExitCode) {
    console.log(`App [${clu.pm2_env.name}:${clu.pm2_env.pm_id}] stopping as ${exit_code} was one of stop_exit_codes [${stopExitCodes.join(',')}]`)
  }
